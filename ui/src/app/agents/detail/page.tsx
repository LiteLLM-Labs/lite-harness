      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-12 border-b border-border flex items-center justify-between px-4 shrink-0">
          <div className="flex items-center gap-2">
            <Button size="sm" variant="ghost" onClick={() => router.push("/agents/")}
        ...
        <main className="flex-1 overflow-y-auto">
          <div className="max-w-4xl mx-auto px-4 py-6 flex flex-col gap-6">
            {error && <Card className="border-destructive p-3"><p className="text-sm text-destructive">{error}</p></Card>}
            {loading && <div className="text-sm text-muted-foreground">Loading…</div>}
            {agent && (<>
              <div className="flex flex-col gap-1">
                <div className="flex items-center gap-2 flex-wrap">
                  <h1 className="text-xl font-semibold">{agent.name}</h1>
                  {agent.model && <span className="text-xs font-mono bg-muted text-muted-foreground rounded px-2 py-0.5">{String(agent.model)}</span>}
                </div>
                {agent.description && <p className="text-sm text-muted-foreground">{agent.description}</p>}
                {agent.created_at && <p className="text-xs text-muted-foreground/60 flex items-center gap-1 mt-1">
                  <Clock className="size-3" />Created {timeAgo(Number(agent.created_at) * 1000)}</p>}
              </div>
              <section>
                ...
                <Card className="p-4">
                  <dl className="grid gap-x-6 gap-y-3 text-sm sm:grid-cols-[140px_1fr]">
                    ...
                    {agent.model && (<><dt className="text-muted-foreground font-medium">Model</dt>
              ...
              <section>
                <div className="flex items-center justify-between mb-2">
                  <h2 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Sessions ({sessions.length})</h2>
                  <Button size="sm" variant="outline" onClick={handleStartSession}><Play className="size-3" />New session</Button>
                </div>
                {sessions.length === 0 ? <p className="text-sm text-muted-foreground">No sessions yet.</p> : (
                  <div className="flex flex-col gap-2">
                    {sessions.map((s) => (
                      <Card key={s.id} className="px-4 py-3 flex items-center justify-between gap-2 cursor-pointer hover:bg-muted/40 transition-colors"
                        ...
                        {s.time?.created && <span className="text-xs text-muted-foreground shrink-0">{timeAgo(s.time.created * 1000)}</span>}
                      </Card>
                    ))}
                  </div>
                )}
...
export default function AgentDetailPage() {
  return <Suspense><AgentDetail /></Suspense>;
}
